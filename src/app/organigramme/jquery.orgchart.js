(function($) {
    ...
    $.fn.orgChart.defaults = {
        ...
        data: [{id:1, name:'Root', description: 'My Organization', localite: 'My Organization', poste: 'My Organization', parent: 0}],
        showControls: false,
        allowEdit: false,
        onAddNode: null,
        onDeleteNode: null,
    ...
    function OrgChart($container, opts){
        ...
        this.startEdit = function(id){
        	//for(i = 0; i<= nodes[id].data.length; i++) {
            	var inputElement = $('<input class="org-input" type="text" value="'+nodes[id].data.i+'"/>');
            	//var inputElement = $('<input class="org-input" type="text" value="'+nodes[id].data.name+'"/>');
            	/*****************  ************************/
            	/*var inputElement3 = $('<input class="org-input" type="text" value="'+nodes[id].data.localite+'"/>');
            	var inputElement4 = $('<input class="org-input" type="text" value="'+nodes[id].data.poste+'"/>');*/
            
            	$container.find('div[node-id='+id+'] h2').replaceWith(inputElement);
            	var commitChange = function(){
                	var h2Element = $('<h2>'+nodes[id].data.i+'</h2>');
                	...
                	/*var h3Element = $('<h2>'+nodes[id].data.localite+'</h2>');
                	if(opts.allowEdit){
                    	h3Element.click(function(){
                        	self.startEdit(id);
                    	})
                	}
                	var h4Element = $('<h2>'+nodes[id].data.poste+'</h2>');
                	if(opts.allowEdit){
                	...
                	/******************** Remplacement des autres champs *******************/
                	inputElement.replaceWith(h2Element);
                	/*inputElement3.replaceWith(h3Element);
                	inputElement4.replaceWith(h4Element);*/
            	}  
            	inputElement.focus();
            	inputElement.keyup(function(event){
                	if(event.which == 13){
                    	...
                    	nodes[id].data.i = inputElement.val();
                    	//nodes[id].data.name = inputElement.val();
                    
                    	/**************** Ajout des champs ********************/
                    	/*nodes[id].data.localite = inputElement.val();
                    	nodes[id].data.poste = inputElement.val();*/
                	}
            	});
        	...
        	this.newNode = function(parentId){
            	...
            	self.addNode({id: nextId, name: '',  description: '', localite: '', poste: '', parent: parentId});
    ...
    function Node(data){
        ...
        this.formatNode = function(opts){
            ...
            /******************** Ajout des controles des champs supplémentaires *****************************/
            if(typeof data.poste !== 'undefined'){
                localiteString = '<h2>'+self.data.localite+'</h2>';
            }
            if(typeof data.poste !== 'undefined'){
                posteString = '<h2>'+self.data.poste+'</h2>';
            }
            /************************************* Fin de l'ajout *******************************************/
            if(typeof data.description !== 'undefined'){
                descString = '<h2>'+self.data.description+'</h2>';
            }
            /*if(typeof data.description !== 'undefined'){
            ...
            /************************************* Et ici on ajoute les variables pour l'affichage *******************************************/
            return "<div class='node' node-id='"+this.data.id+"'> <label>Nom: </label>"+nameString+"<br> <label> Description:</label>"+descString+"<br> <label> Localité:</label>"+localiteString+"<br> <label> Poste:</label>"+posteString+"<br><br>"+buttonsHtml+"</div>";
